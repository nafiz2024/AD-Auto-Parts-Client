"use client";

import { useEffect, useMemo, useState } from "react";
import { usePathname, useRouter, useSearchParams } from "next/navigation";
import { TableRowSkeleton } from "@/components/states/loading-states";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { ConfirmationDialog } from "@/components/ui/confirmation-dialog";
import { EmptyState } from "@/components/ui/empty-state";
import { ErrorState } from "@/components/ui/error-state";
import { TruckIcon } from "@/components/ui/icons";
import { Input } from "@/components/ui/input";
import { PageHeader } from "@/components/ui/page-header";
import { Select } from "@/components/ui/select";
import { routes } from "@/constants/routes";
import { resolveAdminLoadMessage } from "@/features/admin/admin-api-ui";
import { getAdminAccessState } from "@/features/admin/admin-access";
import {
  getAdminCourierList,
  getAdminShipments,
  updateAdminShipmentStatus,
} from "@/features/admin/shipments/admin-shipments-api";
import { useAuth } from "@/hooks/use-auth";
import { useLanguage } from "@/hooks/use-language";
import { useToast } from "@/hooks/use-toast";

function formatDate(value) {
  if (!value) {
    return "--";
  }

  const date = new Date(value);

  if (Number.isNaN(date.getTime())) {
    return value;
  }

  return `${date.toLocaleDateString()} ${date.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  })}`;
}

function updateSearchParams(current, updates) {
  const params = new URLSearchParams(current);

  Object.entries(updates).forEach(([key, value]) => {
    if (value === "" || value === null || value === undefined) {
      params.delete(key);
      return;
    }

    params.set(key, String(value));
  });

  return params.toString();
}

function getStatusVariant(status) {
  const normalized = String(status).toLowerCase();

  if (normalized.includes("deliver")) {
    return "success";
  }

  if (normalized.includes("transit") || normalized.includes("dispatch") || normalized.includes("assign")) {
    return "info";
  }

  if (normalized.includes("fail") || normalized.includes("return")) {
    return "error";
  }

  return "warning";
}

function buildFilters(searchParamsValue) {
  const searchParams = new URLSearchParams(searchParamsValue);

  return {
    page: Math.max(Number.parseInt(searchParams.get("page") || "1", 10) || 1, 1),
    status: searchParams.get("status") || "",
    courier: searchParams.get("courier") || "",
    orderNumber: searchParams.get("orderNumber") || "",
    dateFrom: searchParams.get("dateFrom") || "",
    dateTo: searchParams.get("dateTo") || "",
  };
}

export function AdminShipmentsPage() {
  const auth = useAuth();
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const { t } = useLanguage();
  const toast = useToast();
  const searchKey = searchParams.toString();
  const filters = useMemo(() => buildFilters(searchKey), [searchKey]);
  const [draftState, setDraftState] = useState({
    key: searchKey,
    values: filters,
  });
  const [state, setState] = useState({
    loading: true,
    error: null,
    items: [],
    pagination: null,
    couriers: [],
    selectedId: "",
  });
  const [dialogState, setDialogState] = useState({
    open: false,
    item: null,
    nextStatus: "",
    reason: "",
    submitting: false,
  });
  const [refreshKey, setRefreshKey] = useState(0);
  const draftFilters = draftState.key === searchKey ? draftState.values : filters;

  function updateDraftFilters(updater) {
    setDraftState((current) => ({
      key: searchKey,
      values:
        typeof updater === "function"
          ? updater(current.key === searchKey ? current.values : filters)
          : updater,
    }));
  }

  useEffect(() => {
    if (auth.isLoading) {
      return undefined;
    }

    const access = getAdminAccessState(auth.session);

    if (!access.isAuthenticated) {
      router.replace(routes.admin.adminLogin);
      return undefined;
    }

    if (access.forbidden) {
      auth.logout().finally(() => router.replace(routes.admin.adminLogin));
      return undefined;
    }

    if (access.totpPending) {
      router.replace(routes.admin.adminTotp);
      return undefined;
    }

    let active = true;

    async function loadPage() {
      try {
        if (active) {
          setState((current) => ({ ...current, loading: true, error: null }));
        }
        const [shipments, couriers] = await Promise.all([
          getAdminShipments(filters),
          getAdminCourierList().catch(() => []),
        ]);

        if (active) {
          setState((current) => ({
            loading: false,
            error: null,
            items: shipments.items,
            pagination: shipments.pagination,
            couriers,
            selectedId: current.selectedId || shipments.items[0]?.id || "",
          }));
        }
      } catch (error) {
        if (active) {
          setState({
            loading: false,
            error,
            items: [],
            pagination: null,
            couriers: [],
            selectedId: "",
          });
        }
      }
    }

    loadPage();

    return () => {
      active = false;
    };
  }, [auth, filters, refreshKey, router]);

  function replaceFilters(updates) {
    const query = updateSearchParams(searchParams.toString(), updates);
    const nextHref = query ? `${pathname}?${query}` : pathname;
    const currentHref = searchKey ? `${pathname}?${searchKey}` : pathname;

    if (nextHref !== currentHref) {
      router.replace(nextHref);
    }
  }

  async function handleConfirm() {
    if (!dialogState.item) {
      return;
    }

    setDialogState((current) => ({ ...current, submitting: true }));

    try {
      await updateAdminShipmentStatus(dialogState.item.id, {
        status: dialogState.nextStatus,
        note: dialogState.reason || undefined,
      });
      toast.success(t("shipments"), t("statusUpdatedSuccessfully"));
      setDialogState({ open: false, item: null, nextStatus: "", reason: "", submitting: false });
      setRefreshKey((value) => value + 1);
    } catch (error) {
      setDialogState((current) => ({ ...current, submitting: false }));
      toast.apiError(error, t("shipments"));
    }
  }

  const selectedShipment = state.items.find((item) => item.id === state.selectedId) ?? null;

  if (state.loading) {
    return (
      <div className="space-y-6">
        <PageHeader title={t("shipments")} description={t("adminShipmentsDescription")} />
        <TableRowSkeleton rows={8} />
      </div>
    );
  }

  if (state.error) {
    return (
      <ErrorState
        title={t("failedToLoad")}
        description={resolveAdminLoadMessage(state.error, t("adminShipmentsLoadError"))}
        actionLabel={t("retry")}
        onAction={() => setRefreshKey((value) => value + 1)}
      />
    );
  }

  return (
    <div className="space-y-6">
      <PageHeader title={t("shipments")} description={t("adminShipmentsDescription")} />

      <Card className="space-y-5 rounded-[2rem]">
        <form
          className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"
          onSubmit={(event) => {
            event.preventDefault();
            replaceFilters({
              page: 1,
              status: draftFilters.status,
              courier: draftFilters.courier,
              orderNumber: draftFilters.orderNumber,
              dateFrom: draftFilters.dateFrom,
              dateTo: draftFilters.dateTo,
            });
          }}
        >
          <Select
            value={draftFilters.status}
            onChange={(event) => updateDraftFilters((current) => ({ ...current, status: event.target.value }))}
          >
            <option value="">{t("allShipmentStatuses")}</option>
            <option value="pending">{t("pending")}</option>
            <option value="assigned">{t("assigned")}</option>
            <option value="dispatched">{t("dispatched")}</option>
            <option value="in_transit">{t("inTransit")}</option>
            <option value="out_for_delivery">{t("outForDelivery")}</option>
            <option value="delivered">{t("delivered")}</option>
            <option value="failed">{t("failed")}</option>
            <option value="returned">{t("returned")}</option>
          </Select>
          <Select
            value={draftFilters.courier}
            onChange={(event) => updateDraftFilters((current) => ({ ...current, courier: event.target.value }))}
          >
            <option value="">{t("allCouriers")}</option>
            {state.couriers.map((courier) => (
              <option key={courier.id} value={courier.id}>
                {courier.name}
              </option>
            ))}
          </Select>
          <Input
            value={draftFilters.orderNumber}
            onChange={(event) => updateDraftFilters((current) => ({ ...current, orderNumber: event.target.value }))}
            placeholder={t("searchOrderNumber")}
          />
          <Input
            type="date"
            value={draftFilters.dateFrom}
            onChange={(event) => updateDraftFilters((current) => ({ ...current, dateFrom: event.target.value }))}
          />
          <Input
            type="date"
            value={draftFilters.dateTo}
            onChange={(event) => updateDraftFilters((current) => ({ ...current, dateTo: event.target.value }))}
          />
          <div className="flex flex-wrap gap-3 xl:col-span-4">
            <Button type="submit">{t("applyFilters")}</Button>
            <Button
              type="button"
              variant="outline"
              onClick={() =>
                replaceFilters({
                  page: null,
                  status: null,
                  courier: null,
                  orderNumber: null,
                  dateFrom: null,
                  dateTo: null,
                })
              }
            >
              {t("clearFilters")}
            </Button>
          </div>
        </form>

        {state.items.length === 0 ? (
          <EmptyState
            icon={TruckIcon}
            title={t("shipments")}
            description={t("adminShipmentsEmptyDescription")}
          />
        ) : (
          <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <div className="space-y-4">
              {state.items.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setState((current) => ({ ...current, selectedId: item.id }))}
                  className={`w-full rounded-[1.75rem] border p-4 text-start transition ${
                    state.selectedId === item.id
                      ? "border-brand-red bg-brand-red/5"
                      : "border-border hover:border-brand-red/40"
                  }`}
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="font-semibold text-foreground">{item.shipmentNumber}</p>
                      <p className="text-sm text-muted-foreground">
                        {item.orderNumber} / {item.customerName}
                      </p>
                    </div>
                    <Badge variant={getStatusVariant(item.status)}>{item.statusLabel}</Badge>
                  </div>
                  <div className="mt-3 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                    <p>{t("courier")}: {item.courier}</p>
                    <p>{t("trackingNumber")}: {item.trackingNumber || "--"}</p>
                  </div>
                </button>
              ))}
            </div>

            <Card className="h-fit space-y-4 rounded-[2rem]">
              {selectedShipment ? (
                <>
                  <h2 className="text-xl font-semibold text-foreground">{t("shipmentDetails")}</h2>
                  <div className="space-y-3 text-sm">
                    <div>
                      <p className="text-muted-foreground">{t("orderNumber")}</p>
                      <p className="font-medium text-foreground">{selectedShipment.orderNumber}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">{t("customer")}</p>
                      <p className="font-medium text-foreground">{selectedShipment.customerName}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">{t("courier")}</p>
                      <p className="font-medium text-foreground">{selectedShipment.courier}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">{t("trackingNumber")}</p>
                      <p className="font-medium text-foreground">{selectedShipment.trackingNumber || "--"}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">{t("estimatedDelivery")}</p>
                      <p className="font-medium text-foreground">{selectedShipment.estimatedDelivery || "--"}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">{t("shipmentStatus")}</p>
                      <div className="mt-2">
                        <Badge variant={getStatusVariant(selectedShipment.status)}>{selectedShipment.statusLabel}</Badge>
                      </div>
                    </div>
                    {selectedShipment.note ? (
                      <div>
                        <p className="text-muted-foreground">{t("notes")}</p>
                        <p className="font-medium text-foreground">{selectedShipment.note}</p>
                      </div>
                    ) : null}
                  </div>
                  <div className="space-y-3 border-t border-border pt-4">
                    <Select
                      value=""
                      onChange={(event) =>
                        setDialogState({
                          open: true,
                          item: selectedShipment,
                          nextStatus: event.target.value,
                          reason: "",
                          submitting: false,
                        })
                      }
                    >
                      <option value="">{t("updateShipmentStatus")}</option>
                      <option value="pending">{t("pending")}</option>
                      <option value="assigned">{t("assigned")}</option>
                      <option value="dispatched">{t("dispatched")}</option>
                      <option value="in_transit">{t("inTransit")}</option>
                      <option value="out_for_delivery">{t("outForDelivery")}</option>
                      <option value="delivered">{t("delivered")}</option>
                      <option value="failed">{t("failed")}</option>
                      <option value="returned">{t("returned")}</option>
                    </Select>
                    <p className="text-xs text-muted-foreground">{t("shipmentStatusManagedByBackend")}</p>
                  </div>
                </>
              ) : (
                <EmptyState title={t("shipments")} description={t("selectShipment")} />
              )}
            </Card>
          </div>
        )}
      </Card>

      <ConfirmationDialog
        open={dialogState.open}
        title={t("updateShipmentStatus")}
        description={t("areYouSure")}
        confirmLabel={dialogState.submitting ? t("saving") : t("updateStatus")}
        cancelLabel={t("cancel")}
        onCancel={() => setDialogState({ open: false, item: null, nextStatus: "", reason: "", submitting: false })}
        onConfirm={handleConfirm}
        tone="warning"
        reasonLabel={t("optionalNote")}
        reasonPlaceholder={t("enterStatusReason")}
        reasonValue={dialogState.reason}
        onReasonChange={(reason) => setDialogState((current) => ({ ...current, reason }))}
      />
    </div>
  );
}
